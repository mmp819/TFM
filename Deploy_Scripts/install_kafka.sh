...
install_kafka() {
    ...
    echo $PASS | sshpass -p $PASS ssh -tt -p $PORT $USER@$node "echo 'listeners=PLAINTEXT://0.0.0.0:9092' | sudo tee -a /opt/kafka/config/server.properties && echo 'advertised.listeners=PLAINTEXT://10.0.3.10:9092' | sudo tee -a /opt/kafka/config/server.properties"
}
